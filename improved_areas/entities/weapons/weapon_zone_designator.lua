...
function SWEP:Deploy()
	if SERVER then
		...
		for k,v in pairs(zones.List) do
			for k2,v2 in pairs(v.data.points)do
...
function SWEP:DrawHUD()
	local zone,id = LocalPlayer():GetCurrentZone(GetConVarNumber("zone_filter") == 1 and self:GetZoneClass() or nil)
    local class = zone and zone.data.class.."(# "..id..")" or "NONE"
	surface.SetFont("DermaLarge")
	local w = math.max(360,surface.GetTextSize("Current Zone: "..class)+40)
	draw.RoundedBox(8,ScrW()/2-w/2,ScrH()-160,w,100,Color(0,0,0,150))
    draw.SimpleText("Current Zone: "..class, "DermaLarge", ScrW()/2,ScrH()-130,color_white,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
...
function SWEP:PrimaryAttack()
	...
	elseif mode == 3 then //Edit
		...
		if IsValid(curr) then
			...
			zones.List[curr:GetZoneID()].data.height[curr:GetAreaNumber()] = curr:GetTall()
	...
	elseif mode == 4 then
		...
		elseif IsValid(curr) then
			...
			zones.List[id].data.points[areanum] = pts
...
function SWEP:SecondaryAttack()
	...
	if mode == 1 then //delete
		...
		if !tr.HitWorld and IsValid(tr.Entity) and tr.Entity:GetClass() == "ent_zone_point" then
			...
			if id != -1 then
				...
				if #zones.List[id].data.points > 1 then
	...
	elseif mode == 2 then //split
		...
		if !tr.HitWorld and IsValid(tr.Entity) and tr.Entity:GetClass() == "ent_zone_point" then
			...
			repeat
				...
				next:SetAreaNumber(next:GetAreaNumber() - #zones.List[id].data.points)
	...
	elseif mode == 4 then //Remove a point
		...
		if !tr.HitWorld and IsValid(tr.Entity) and tr.Entity:GetClass() == "ent_zone_point" or IsValid(self:GetCurrentPoint()) then
			...
			if #zones.List[id].data.points[areanum] > 3 then
			...
			if curr:GetZoneID() != trent:GetZoneID() then
				...
				for k,next in pairs(ents.FindByClass("ent_zone_point")) do
					if next:GetZoneID() == cid then
						...
						next:SetAreaNumber(next:GetAreaNumber() + #zones.List[trent:GetZoneID()].data.points)
